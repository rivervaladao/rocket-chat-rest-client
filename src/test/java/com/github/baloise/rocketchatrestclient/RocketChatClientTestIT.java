...
public class RocketChatClientTestIT {
	...
	public void testCreateCloseAndOpenChannel() throws Exception {
		...
		assertTrue("Room Id shouldn't be null if the room was created",
				channel.getId() != null && !channel.getId().isEmpty());
	...
	public void testListSettings() throws Exception {
		...
		Setting organizationNameSetting = _settings.stream().filter(e -> "Organization_Name".equals(e.getId()))
				.findFirst().orElse(null);
		assertNotNull(organizationNameSetting);
	}

	@Test
	public void testUsersUpdateAndDelete() {
		...
		try {
			 ...
			 * Create a user
			 */
			User[] users = rc.getUsersApi().list();
			User user = null;
			...
			if (user == null) {
				...
				user = rc.getUsersApi().create(createUserRequest);
			}
			assertNotNull(user.getId());

			/**
			 * Verify the user exists.
			 * Test new API users.list with Query filter hash.
			 */
			User[] createdUsers = rc.getUsersApi()
					.list(new RocketChatQueryParams("query", "{\"username\": \"" + TEST_USERNAME + "\"}"));
			assertTrue(createdUsers != null && createdUsers.length > 0 && TEST_USERNAME.equals(createdUsers[0].getUsername()));
			...
			final User updatedUser = rc.getUsersApi().update(updateUserRequest);
			 ...
			 * Delete a user
			 */
			final boolean deleted = rc.getUsersApi().deleteByUserIdUserId((new UserIdRequest("", user.getId())));
			assertTrue(deleted);
